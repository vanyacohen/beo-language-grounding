...
def eval(model, device, test_data, writer, epoch, prefix):
    ...
    with torch.no_grad():
        for (id, bow, vec, val, bow_adjs, beo_adjs) in test_data:
            ...
            # accuracy
            bow_p = binarize_prediction(nn.Softmax(dim=1)(bow_p)[0])
            beo_p = binarize_prediction(nn.Softmax(dim=1)(beo_p)[0])
    ...
    with torch.no_grad():
        for (label, bow, vecs) in ret_data:
            ...
            if 0 == indexes[0]:
                ...
                correct2 += 1
    print('Ret {} {}'.format(correct / len(ret_data), correct2 / len(ret_data)))
...
def csvTrainRowsToDataset(datarows, id2vec):
    avgLabel = {}
    for row in datarows:
        for i in range(6):
    ...
    for row in datarows:
        if row[0] in id2vec:
            for vector in id2vec[row[0]]:
                dataset.append((row[0], row[7].translate(str.maketrans('', '', string.punctuation)).lower(), vector, list(map(binarize, avgLabel[row[0]]))))
    return dataset
...
def csvRowsToDataset(datarows, id2vec):
    ...
    for row in datarows:
        if row[0] in id2vec:
            dataset.append((row[0], row[7].translate(str.maketrans('', '', string.punctuation)).lower(), id2vec[row[0]], list(map(binarize, avgLabel[row[0]]))))
    return dataset
...
# sample a number of predictions for each object ID
def sample(num, id2vec, train_pos, train_neg):
    train_data = []
    for i in id2vec:
        vectors = id2vec[i]
        selected = random.sample(vectors, num)
        for s in selected:
            for row in train_pos:
                if ((row[0] == i) and np.allclose(row[2], s)):
            ...
            with open(opt.testdata, 'r') as testdatafile:
                with open(opt.couchvectors, 'r') as vectorfile:
                    with open(opt.testvectors, 'r') as testvectorfile:
                        ...
                        testvectorrows = csv.reader(testvectorfile)

                        # make a dict of train ids to numpy beo vecs
                        make_vector = lambda s: np.fromstring(s.replace('\n', '')[1:-1], dtype=np.float, sep=' ')
                        id2vec = {}
                        for row in vectorrows:
                            if not row[0] in id2vec:
                                id2vec[row[0]] = []
                            id2vec[row[0]].append(make_vector(row[1]))

                        # make a dict of test ids to numpy beo vecs
                        testid2vec = {row[0] : make_vector(row[1]) for row in testvectorrows}

                        # find the avgs of the labelings
                        train_data = csvTrainRowsToDataset(traindatarows, id2vec)
                        dev_data = csvRowsToDataset(devdatarows, testid2vec)
                        test_data = csvRowsToDataset(testdatarows, testid2vec)
                        ...
                        for epoch in range(0, 100):
                            print("Epoch ", epoch + 1)
                            train_data = sample(NUM_SAMPLE, id2vec, train_pos, train_neg)
